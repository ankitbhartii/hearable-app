...
import { LogOut, ArrowRight, Play, Pause, ChevronLeft, ChevronRight, Trash2, Volume2, VolumeX, Search, Bookmark, Info, ListOrdered } from 'lucide-react'
import { useRouter } from 'next/navigation'
import { motion, AnimatePresence } from 'framer-motion'
import Loader from '../Loader'

// Memoized Sub-Component Card to isolate DOM rendering chains
const AudiobookCard = memo(({ book, isSaved, isCurrentlyPlaying, isPlaying, onToggleBookmark, onDelete, onPlayClick, onExpandView }) => {
  return (
    <motion.div 
    ...
    >
      <div className="relative aspect-[16/10] w-full bg-neutral-900 overflow-hidden">
        ...
        <div className="absolute inset-0 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity duration-150 bg-black/40 backdrop-blur-[1px]">
          <button 
            onClick={(e) => { e.stopPropagation(); onPlayClick(book); }}
        ...
        <button
          onClick={(e) => onToggleBookmark(book.id, e)}
...
export default function Dashboard() {
  ...
  // App Core Functional State Architecture
  ...
  // Navigation & Viewport State Engine
  ...
  // Media Performance Track States
  ...
  useEffect(() => {
    async function fetchData() {
      try {
        ...
        const [genresRes, booksRes, libRes] = await Promise.all([
  ...
  useEffect(() => {
    if (currentActiveChapter && activeBookContext && 'mediaSession' in navigator) {
      navigator.mediaSession.metadata = new MediaMetadata({
        title: `${currentActiveChapter.title} - ${activeBookContext.title}`.toUpperCase(),
        artist: activeBookContext.author.toUpperCase(),
        album: 'HEARABLE VAULT',
        artwork: [
          { src: activeBookContext.cover_url, sizes: '512x512', type: 'image/jpeg' },
        ]
      });
  ...
  }, [currentActiveChapter, activeBookContext]);
  ...
  const handleTimeUpdate = () => {
    ...
    if (persistentScrubberRef.current) {
      persistentScrubberRef.current.style.width = `${(current / duration) * 100}%`
    }
    if (expandedCurrentTimeTextRef.current) {
      expandedCurrentTimeTextRef.current.innerText = formatTime(current)
    }

    if (Math.floor(current) % 6 === 0 && currentActiveChapter?.id) {
      localStorage.setItem(`hearable_resume_${currentActiveChapter.id}`, current)
    }
  }
  ...
  const handleLoadedMetadata = () => {
    ...
    if (rangeScrubberRef.current) rangeScrubberRef.current.max = duration
    if (expandedDurationTextRef.current) expandedDurationTextRef.current.innerText = formatTime(duration)
    ...
    if (currentActiveChapter?.id) {
      ...
      if (savedTime) {
        ...
        if (parsedTime > 2 && parsedTime < duration - 10) {
    ...
    if (audio.paused) {
      ...
      if (playPromiseRef.current) {
        playPromiseRef.current
          .then(() => { audio.pause(); setIsPlaying(false) })
          .catch(() => { audio.pause(); setIsPlaying(false) })
      } else {
        audio.pause()
        setIsPlaying(false)
      }
  ...
  const handleDelete = async (bookId, e) => {
    e.stopPropagation()
    if (!window.confirm("WARNING: Are you sure you want to permanently purge this Master Book and ALL its media?")) return
    ...
    try {
      ...
      if (activeBookContext?.id === bookId) {
        setActiveBookContext(null)
        setCurrentActiveChapter(null)
  ...
  // ==========================================
  // FETCH CHAPTERS AND OPEN IMMERSIVE PLAYER (WITH STANDALONE FIX)
  // ==========================================
  const handleTrackCardSelect = async (book) => {
    ...
    try {
      ...
      if (data && data.length > 0) {
        // Normal Episodic Series Setup
        setBookChapters(data)
        if (!currentActiveChapter || currentActiveChapter.book_id !== book.id) {
          setCurrentActiveChapter(data[0])
        }
      } else if (book.audio_url && book.audio_url !== 'EPISODIC_SERIES') {
        // NEW: Standalone Virtual Chapter Fix
        const standaloneVirtualChapter = {
          id: book.id, 
          book_id: book.id,
          chapter_number: 1,
        ...
        if (!currentActiveChapter || currentActiveChapter.id !== book.id) {
          setCurrentActiveChapter(standaloneVirtualChapter)
        }
      } else {
        setBookChapters([])
      }
  ...
  const handlePlayFromMainCard = (book) => {
      if (activeBookContext?.id === book.id && isPlaying) {
          togglePlayPause();
      } else {
          handleTrackCardSelect(book);
      }
  }

  const handleChapterSelect = (chapter) => {
      if (currentActiveChapter?.id === chapter.id) {
          togglePlayPause();
      } else {
  ...
  const featuredHeroBook = audiobooks[0] || {
    ...
    description: "Immerse yourself inside exceptional creative literature. Explore our new expansive catalog library categories.",
  ...
  return (
    <div className="min-h-screen bg-[#141414] text-white font-['Hanken_Grotesk'] antialiased pb-40 select-none transition-colors duration-300 overflow-x-hidden">
      ...
      {currentActiveChapter && (
        <audio 
          ...
          onEnded={() => {
              ...
              if (currentIndex >= 0 && currentIndex < bookChapters.length - 1) {
                  setCurrentActiveChapter(bookChapters[currentIndex + 1]);
              } else {
                  setIsPlaying(false);
              }
          }}
      ...
      <header className="bg-gradient-to-b from-black via-black/70 to-transparent fixed top-0 left-0 right-0 z-50 px-6 md:px-12 py-5 flex flex-col md:flex-row justify-between items-start md:items-center gap-4 transition-all">
        <div className="flex items-center gap-8 w-full md:w-auto justify-between md:justify-start">
          <div className="flex items-center gap-2 cursor-pointer select-none" onClick={() => { setActiveView('catalog'); setSearchQuery(''); }}>
            ...
            <span className="bg-[#e50914] text-white text-[8px] font-[900] px-1.5 py-0.5 rounded-sm ml-1 tracking-widest uppercase">V3.5</span>
          </div>
      ...
      {/* ================= NETFLIX WIDESCREEN HERO BILLBOARD ================= */}
        ...
        <div className="relative w-full h-[56vw] min-h-[420px] max-h-[780px] bg-black overflow-hidden transform-gpu select-none">
          ...
          <div className="absolute left-6 md:left-12 bottom-[30%] md:bottom-[22%] max-w-xl space-y-4 md:space-y-5 z-10">
            ...
            <div className="flex items-center gap-3 pt-2">
              <button 
                onClick={() => handlePlayFromMainCard(featuredHeroBook)}
      ...
      {errorDebug && <div className="mx-6 md:mx-12 my-6 p-4 bg-[#e50914]/20 border-l-4 border-[#e50914] text-white text-sm font-bold uppercase tracking-wide rounded-r-md">SYSTEM FAULT: {errorDebug}</div>}

      {/* ================= CAROUSEL ROWS DISPLAY SHELF ================= */}
          ...
          return (
            <section key={genre.id} className="space-y-3 relative pb-8">
              <div className="flex items-baseline gap-2">
                <h2 className="text-[14px] sm:text-[18px] md:text-[22px] font-bold tracking-tight text-white leading-none hover:text-[#e50914] cursor-pointer transition-colors antialiased">{genre.name}</h2>
                <span className="font-bold text-[10px] text-neutral-500">[{rowBooks.length}]</span>
              </div>
              ...
              <div className="flex gap-4 overflow-x-auto pb-4 pt-1 snap-x scroll-smooth no-scrollbar transform-gpu will-change-transform">
                {rowBooks.map((book) => (
                  <AudiobookCard 
                    key={book.id}
                    book={book}
                    isSaved={library.includes(book.id)}
                    isCurrentlyPlaying={activeBookContext?.id === book.id && isPlaying}
                    isPlaying={isPlaying}
                    onToggleBookmark={handleToggleBookmark}
                    onDelete={handleDelete}
                    onPlayClick={handlePlayFromMainCard}
                    onExpandView={handleTrackCardSelect}
                  />
                ))}
              </div>
            </section>
      ...
      {/* ================= FIXED FLOATING BASE DOCKBAR CONTROLS ================= */}
      <AnimatePresence>
        {currentActiveChapter && activeBookContext && !isPlayerExpanded && (
          ...
          >
            <div className="absolute top-0 left-0 right-0 h-[2px] bg-white/10">
              <div ref={persistentScrubberRef} className="h-full bg-[#e50914] w-0 transition-all duration-300" />
            </div>
            ...
            <div className="flex items-center gap-4 w-full sm:w-1/2 cursor-pointer" onClick={() => setIsPlayerExpanded(true)}>
              <div className="relative h-12 w-12 flex-shrink-0 bg-neutral-900 border border-white/10 rounded-md overflow-hidden">
                <img src={activeBookContext.cover_url} alt="" className="h-full w-full object-cover" />
              </div>
              <div className="min-w-0">
                <h4 className="font-bold text-sm text-white tracking-tight uppercase truncate">{currentActiveChapter.title}</h4>
                <p className="text-[#e50914] font-bold text-xs uppercase mt-0.5 tracking-wider truncate">
                   CH. {currentActiveChapter.chapter_number} // {activeBookContext.title}
                </p>
              </div>
            </div>
            <div className="w-full sm:w-1/2 flex items-center justify-end gap-4">
              <button onClick={togglePlayPause} className="bg-white text-black p-2.5 rounded-full hover:bg-[#e50914] hover:text-white transition-colors flex items-center justify-center shadow-md active:scale-95 border-none outline-none">
                {isPlaying ? <Pause className="h-4 w-4 fill-current" /> : <Play className="h-4 w-4 fill-current ml-0.5" />}
              </button>
              <div onClick={() => setIsPlayerExpanded(true)} className="bg-[#e50914] hover:bg-[#b81d24] text-white text-xs font-bold uppercase tracking-widest px-5 py-3 rounded-md shadow-lg cursor-pointer flex items-center gap-2">
      ...
      {/* ================= EPISODIC CINEMATIC INTERFACE CONTROLLER ================= */}
      <AnimatePresence>
        {activeBookContext && isPlayerExpanded && (
          <motion.div 
            ...
            className="fixed inset-0 bg-gradient-to-b from-[#0f0f0f] to-[#050404] z-[100] flex flex-col p-6 md:p-12 select-none overflow-y-auto"
          >
            <div className="w-full flex items-center justify-between z-10 flex-shrink-0 mb-8">
              <button onClick={() => setIsPlayerExpanded(false)} className="bg-black/40 border border-white/10 text-white p-3 rounded-full hover:bg-white hover:text-black transition-all transform hover:scale-105 active:scale-95">
            ...
            <div className="w-full max-w-5xl mx-auto flex flex-col md:flex-row gap-12 items-start z-10 pb-12 border-b border-white/10">
              ...
              <div className="text-left space-y-5 flex-1">
                ...
                <div className="flex flex-wrap items-center gap-3">
                    <span className="bg-white/5 border border-white/10 text-neutral-300 px-3 py-1 font-mono text-[10px] uppercase tracking-widest rounded-md">
                    {activeBookContext.genre_slug}
                    </span>
                    <span className="text-neutral-500 font-bold text-[10px] uppercase tracking-widest">
                        {bookChapters.length === 1 && bookChapters[0].title === 'Complete Audio' ? '1 STANDALONE TRACK' : `${bookChapters.length} CHAPTERS`}
                    </span>
                </div>
                
                {activeBookContext.description && (
                  <p className="text-neutral-400 text-sm md:text-[15px] leading-relaxed uppercase font-medium tracking-wide">
                    {activeBookContext.description}
                  </p>
                )}
                ...
                {bookChapters.length > 0 && (
                    <button 
                        onClick={() => {
                            if (!currentActiveChapter) setCurrentActiveChapter(bookChapters[0]);
                            else togglePlayPause();
                        }} 
                        className="mt-4 bg-white hover:bg-neutral-200 text-black px-8 py-3 rounded-md font-extrabold text-sm uppercase tracking-wider transition-all flex items-center gap-2 active:scale-95"
                    >
            ...
            <div className="w-full max-w-5xl mx-auto py-10 z-10 flex-1">
                <div className="flex items-center justify-between mb-6">
                    <h2 className="text-xl font-bold text-white uppercase tracking-wider antialiased">Episodes</h2>
                </div>

                {bookChapters.length === 0 ? (
                    <div className="p-8 border border-neutral-800 border-dashed rounded-xl bg-black/20 text-center">
                        <ListOrdered className="w-8 h-8 text-neutral-600 mx-auto mb-3" />
                        <p className="text-neutral-400 font-bold text-sm uppercase tracking-widest">No audio indexed for this title yet.</p>
                    </div>
                ) : (
                        ...
                        {bookChapters.map((chapter) => {
                            ...
                            return (
                                <div 
                                    ...
                                    className={`group flex items-center gap-4 p-4 rounded-xl cursor-pointer transition-all border ${isThisChapterPlaying ? 'bg-white/10 border-white/20' : 'bg-transparent border-transparent hover:bg-white/5'}`}
                                >
                                    ...
                                    <div className="flex-1">
                                        <h4 className={`font-bold text-sm md:text-base tracking-wide uppercase transition-colors ${isThisChapterPlaying ? 'text-white' : 'text-neutral-300 group-hover:text-white'}`}>
                                            {chapter.title}
                                        </h4>
                                    </div>
                                    
                                    <div className="text-xs font-mono font-bold text-neutral-500 uppercase tracking-widest hidden sm:block">
                                        {chapter.duration || 'AUDIO'}
                                    </div>
                                    
                                    <div className="opacity-0 group-hover:opacity-100 transition-opacity pr-2">
                                         {isThisChapterPlaying && isPlaying ? <Pause className="w-5 h-5 text-white" /> : <Play className="w-5 h-5 text-white" />}
                                    </div>
                                </div>
                            )
                        })}
            ...
            <AnimatePresence>
                {currentActiveChapter && (
                    <motion.div 
                        ...
                        className="sticky bottom-0 w-full max-w-4xl mx-auto bg-[#181818]/80 border border-white/10 p-6 md:p-8 rounded-2xl backdrop-blur-xl z-20 mt-auto shadow-2xl flex-shrink-0"
                    >
                        <div className="space-y-2">
                        ...
                        <div className="flex flex-col sm:flex-row items-center justify-between gap-6 py-2 mt-4">
                            <div className="flex items-center gap-3 w-full sm:w-1/3 justify-center sm:justify-start">
                                ...
                                <input type="range" min="0" max="1" step="0.01" value={isMuted ? 0 : volume} onChange={handleVolumeChange} className="w-24 h-1 bg-white/20 appearance-none rounded-full volume-scrubber outline-none" />
                            </div>

                            <div className="flex items-center justify-center gap-6 md:gap-8 w-full sm:w-1/3">
                                <button onClick={() => { if(audioRef.current) audioRef.current.currentTime -= 15 }} className="text-neutral-400 hover:text-white p-2"><ChevronLeft className="w-7 h-7" /></button>
                                ...
                                <button onClick={() => { if(audioRef.current) audioRef.current.currentTime += 15 }} className="text-neutral-400 hover:text-white p-2"><ChevronRight className="w-7 h-7" /></button>
                            </div>

                            <div className="flex items-center justify-center sm:justify-end w-full sm:w-1/3">
                                <button onClick={handleSpeedToggle} className="text-xs font-bold bg-black/40 border border-white/10 text-neutral-300 hover:text-white px-4 py-2 rounded-md w-16 text-center">{playbackRate}X</button>
                            </div>
                        </div>
                    </motion.div>
                )}
