...
import { LayoutDashboard, UploadCloud, ImageIcon, Film, Type, User, FileText, ListOrdered, BookOpen, Disc, Layers } from 'lucide-react'
...
export default function AdminConsole() {
  ...
  // NEW: Format Selector (Movie vs Series)
  const [bookFormat, setBookFormat] = useState('episodic') // 'single' | 'episodic'
  ...
  // ==========================================
  // PUBLISH MASTER BOOK RECORD (Movie or Series)
  // ==========================================
  const handlePublishBook = async (e) => {
    ...
    try {
      ...
      let finalAudioUrl = 'EPISODIC_SERIES' // Default bypass for DB Not-Null constraint

      // 2. If Standalone Format, Upload the single audio file now
      if (bookFormat === 'single') {
        ...
        const cloudinaryAudioResponse = await new Promise((resolve, reject) => {
          ...
          xhr.onload = () => { if (xhr.status === 200) resolve(JSON.parse(xhr.responseText)); else reject(new Error('Cloudinary audio stream rejected')); }
          xhr.onerror = () => reject(new Error('Network break'))
      ...
      if (bookFormat === 'single') {
        setStatusMessage('SUCCESS: STANDALONE AUDIOBOOK DEPLOYED.')
      } else {
        setStatusMessage('SUCCESS: SERIES RECORD CREATED. PROCEED TO UPLOAD CHAPTERS.')
        setActiveTab('chapter') // Auto route to chapters tab
      }
  ...
  const handlePublishChapter = async (e) => {
    ...
    try {
      ...
      const cloudinaryResponse = await new Promise((resolve, reject) => {
        ...
        xhr.onload = () => { if (xhr.status === 200) resolve(JSON.parse(xhr.responseText)); else reject(new Error('Cloudinary stream rejected')); }
        xhr.onerror = () => reject(new Error('Network break'))
        xhr.send(audioFormData)
      })
  ...
  return (
    <div className="min-h-screen bg-[#141414] text-white font-['Hanken_Grotesk'] antialiased pb-24 select-none overflow-x-hidden">
      <style jsx global>{`@import url('https://fonts.googleapis.com/css2?family=Hanken+Grotesk:wght@400;700;900&display=swap');`}</style>
      ...
      <header className="bg-gradient-to-b from-black to-transparent backdrop-blur-md px-6 md:px-12 py-5 flex justify-between items-center sticky top-0 z-40 border-b border-white/5">
        <div className="flex items-center gap-2">
          ...
          <span className="bg-[#e50914] text-white text-[8px] font-[900] px-1.5 py-0.5 rounded-sm ml-2 tracking-widest uppercase">V3.5 (HYBRID)</span>
        </div>
      ...
      <main className="max-w-4xl mx-auto px-6 py-12">
        <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="bg-[#181818]/60 border border-white/10 rounded-2xl backdrop-blur-md shadow-2xl overflow-hidden">
          ...
          <div className="flex border-b border-white/10">
            <button onClick={() => setActiveTab('book')} className={`flex-1 py-5 text-sm font-bold uppercase tracking-widest flex items-center justify-center gap-2 transition-colors ${activeTab === 'book' ? 'bg-[#e50914] text-white' : 'text-neutral-400 hover:bg-white/5 hover:text-white'}`}>
              <BookOpen className="w-4 h-4" /> Master Record
            </button>
            <button onClick={() => setActiveTab('chapter')} className={`flex-1 py-5 text-sm font-bold uppercase tracking-widest flex items-center justify-center gap-2 transition-colors ${activeTab === 'chapter' ? 'bg-[#e50914] text-white' : 'text-neutral-400 hover:bg-white/5 hover:text-white'}`}>
              <ListOrdered className="w-4 h-4" /> Append Chapters
            </button>
          </div>
            ...
            {activeTab === 'book' && (
              <form onSubmit={handlePublishBook} className="space-y-6">
                ...
                <div className="bg-black/40 p-1.5 rounded-xl border border-white/10 flex">
                  <button type="button" onClick={() => setBookFormat('single')} className={`flex-1 py-3 text-xs font-bold uppercase tracking-widest rounded-lg flex items-center justify-center gap-2 transition-all ${bookFormat === 'single' ? 'bg-white text-black shadow-md' : 'text-neutral-500 hover:text-white'}`}>
                    <Disc className="w-4 h-4" /> Standalone (Movie)
                  </button>
                  <button type="button" onClick={() => setBookFormat('episodic')} className={`flex-1 py-3 text-xs font-bold uppercase tracking-widest rounded-lg flex items-center justify-center gap-2 transition-all ${bookFormat === 'episodic' ? 'bg-white text-black shadow-md' : 'text-neutral-500 hover:text-white'}`}>
                    <Layers className="w-4 h-4" /> Episodic (Series)
                  </button>
                </div>

                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  <div className="space-y-2">
                    <label className="flex items-center gap-2 font-bold text-xs text-neutral-400 uppercase tracking-widest"><Type className="w-3.5 h-3.5" /> Book Title *</label>
                    <input type="text" required value={title} onChange={(e) => setTitle(e.target.value)} className="w-full bg-black/40 text-white p-4 border-2 border-neutral-800 rounded-xl focus:border-[#e50914] focus:outline-none uppercase text-sm font-bold" />
                  </div>
                  <div className="space-y-2">
                    <label className="flex items-center gap-2 font-bold text-xs text-neutral-400 uppercase tracking-widest"><User className="w-3.5 h-3.5" /> Author Name *</label>
                    <input type="text" required value={author} onChange={(e) => setAuthor(e.target.value)} className="w-full bg-black/40 text-white p-4 border-2 border-neutral-800 rounded-xl focus:border-[#e50914] focus:outline-none uppercase text-sm font-bold" />
                  </div>
                </div>
                  ...
                  <div className="space-y-3">
                    ...
                    <div className="relative w-full bg-[#e50914]/10 border-2 border-[#e50914]/30 border-dashed rounded-xl p-8 flex flex-col items-center justify-center text-center hover:border-[#e50914] transition-colors">
                      ...
                      <p className="font-bold text-sm text-white uppercase tracking-wide">
                        {masterAudioFile ? masterAudioFile.name : 'BROWSE FULL LENGTH AUDIO'}
                      </p>
                    </div>
                  </div>
                )}
                ...
                <div className="space-y-2">
                  <label className="flex items-center gap-2 font-bold text-xs text-neutral-400 uppercase tracking-widest"><FileText className="w-3.5 h-3.5" /> Description</label>
                  <textarea rows="3" value={description} onChange={(e) => setDescription(e.target.value)} className="w-full bg-black/40 text-white p-4 border-2 border-neutral-800 rounded-xl focus:border-[#e50914] focus:outline-none uppercase text-sm font-bold resize-none" />
                </div>

                <button type="submit" disabled={publishing} className="w-full bg-[#e50914] hover:bg-[#b81d24] py-4 rounded-md font-bold text-sm uppercase tracking-wider transition-all shadow-[0_4px_20px_rgba(229,9,20,0.25)]">
                  {publishing ? 'PROCESSING...' : (bookFormat === 'single' ? 'Deploy Standalone Asset' : '1. Create Series Master Record')}
                </button>
              </form>
            )}
              ...
              <form onSubmit={handlePublishChapter} className="space-y-6">
                ...
                <div className="space-y-2">
                  ...
                  {existingBooks.length === 0 ? (
                    <div className="p-4 bg-white/5 text-neutral-400 text-sm border-2 border-dashed border-neutral-800 rounded-xl uppercase font-bold text-center">No Books Found. Create a Master Book First.</div>
                  ) : (
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  <div className="space-y-2">
                    <label className="font-bold text-xs text-neutral-400 uppercase tracking-widest">Chapter Number *</label>
                    <input type="number" min="1" required value={chapterNumber} onChange={(e) => setChapterNumber(e.target.value)} placeholder="E.G., 1" className="w-full bg-black/40 text-white p-4 border-2 border-neutral-800 rounded-xl focus:border-[#e50914] focus:outline-none text-sm font-bold" />
                  </div>
                  <div className="space-y-2">
                    <label className="font-bold text-xs text-neutral-400 uppercase tracking-widest">Chapter Title</label>
                    <input type="text" value={chapterTitle} onChange={(e) => setChapterTitle(e.target.value)} placeholder="E.G., THE BEGINNING" className="w-full bg-black/40 text-white p-4 border-2 border-neutral-800 rounded-xl focus:border-[#e50914] focus:outline-none uppercase text-sm font-bold" />
                  </div>
                </div>
                ...
                <div className="space-y-3">
                  ...
                  <div className="relative w-full bg-black/40 border-2 border-neutral-800 border-dashed rounded-xl p-8 flex flex-col items-center justify-center text-center hover:border-[#e50914]/50 transition-colors">
                    ...
                    <p className="font-bold text-sm text-white uppercase tracking-wide">
                      {audioFile ? audioFile.name : 'BROWSE LOCAL AUDIO FILE'}
                    </p>
                  </div>
                </div>

                <button type="submit" disabled={publishing || existingBooks.length === 0} className="w-full bg-white text-black hover:bg-neutral-200 py-4 rounded-md font-extrabold text-sm uppercase tracking-wider transition-all disabled:opacity-30">
                  {publishing ? 'STREAMING...' : '2. Append Chapter to Book'}
                </button>
              </form>
            )}
